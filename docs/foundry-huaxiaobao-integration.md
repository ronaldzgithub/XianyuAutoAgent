...
默认 SQLite journal 是 `data/huaxiaobao_adapter.db`，可用 `XIANYU_ADAPTER_STATE_PATH` 指向 Huaxiaobao 管理的持久卷。原生监听器与 adapter 共用此 journal：入站咨询在模型调用前原子写入 `OBSERVED`，随后领取有期限的 `PROCESSING` lease，只有绑定同一账号、会话、message ref/revision 和 lease 的 checkpoint ACK 才进入 `PROCESSED`。进程在观察后崩溃时，新实例不会把 observation 冒充完成；旧 lease 到期后可重新领取。重复 claim/ACK 幂等，过期或被人工接管撤销的 lease、旧 revision 和错误绑定均 fail closed。

人工接管也写入同库，只有明确的 desired-state 命令才能改变状态：`MANUAL_TAKEOVER_KEYWORDS` 进入暂停，`MANUAL_RESUME_KEYWORDS` 恢复；命令与原生控制消息及 revision 绑定，重复或迟到重放不会执行反向 toggle。暂停会撤销未完成 lease，暂停期间不能领取；恢复后保留的 `OBSERVED` 咨询可继续领取。进程重启不会清除暂停。同一 operation ID 与同一内容重放返回缓存结果；同 ID 不同内容 fail closed。请求内禁止携带 Cookie、Token、密码等凭据材料。该 journal 是工具执行状态，不是商业账本或人工验收记录。
...
- 真实账号、平台准入和获批发送：未验证。
- 持久化人工任务、工具复查和业务 ACK：无真实账号证据；原生咨询 observation/lease/checkpoint ACK、暂停撤权与重启恢复、adapter 查询、重复结果与幂等冲突仅有离线单元测试证据。
